...
class Video_API {
    ...
    static func downloadVideoWithProgress(delegate: AppDelegate, video: Data?, image: Data, callback: @escaping (MessageVideoProgress) -> ()){
    ...
    static func downloadVideo(delegate: AppDelegate, video: Data?, image: Data, callback: @escaping (MessageVideoModel) -> ()){
    ...
    static func requestHistory(delegate: AppDelegate, email: String, callback: @escaping (HistorysVideoModel) -> ()){
    ...
    static func requestConfirmToSave(delegate: AppDelegate, hash: String, circle: String, callback: @escaping (MessageModel) -> ()){
    ...
    static func requestGetSympathy(delegate: AppDelegate, hash: String, callback: @escaping (Sympathy_Model) ->()){
        delegate.providerVideo.rx.request(.getSympathy(hash: hash)).asObservable().subscribe(onNext: {  (responce) in
            if responce.statusCode >= 200 && responce.statusCode < 300{
                let dataResoponse = responce.data
                if let strData = String(data: dataResoponse, encoding: String.Encoding.utf8){
                    if let h = Mapper<Sympathy_Model>().map(JSONString: strData){
                        callback(h)
                        return
                    }
                }
            }
            callback(Sympathy_Model())
            return
        }, onError: { (error) in
            let msg = MessageModel()
            msg.code = 500
    ...
    //    MARK:- Получить координаты лайков
    static func requestGetNewSympathy(delegate: AppDelegate, hash: String, callback: @escaping (Sympathy_Model) ->()){
...
//    MARK:- отправить координаты лайка на сервер
    static func requestPutSympathy(delegate: AppDelegate, hash: String, action: Sympathy, cx: String, cy: String, callback: @escaping (MessageModel) -> ()){
        ...
        //    MARK:- удалить видео
    static func requestDeleteVideo(delegate: AppDelegate, hash: String, callback: @escaping (MessageModel)->()){
