...
class ImagePlayerViewController: UIViewController {
    ...
    var duration = 5000.0
    var updateTime : ((Double, Double) -> ())? = nil //передаем контроллеру воспроизведения время воспроизведения
    var videoEnd : ((Bool) -> ())? = nil //передаем контроллеру воспроизведения состояние остановки видео
    var videoPrevious : (() -> ())? = nil //передаем контроллеру воспроизведения состояние проигрывание предедущего видео
    ...
    private func downloadPicture(urlString: String?){
        if let urlImg = urlString{
            if let uTime = self.updateTime{
                ...
                imageView.sd_setImage(with: URL(string: "\(urlImg)")) { [weak self] (img, error, cache, url) in
                var iter = 0
                self?.timerImagePlay = Repeater.every(.milliseconds(10), count: 500, queue: DispatchQueue.main, { [weak self] (val) in
                    iter += 1
                    if let uTime = self?.updateTime{
