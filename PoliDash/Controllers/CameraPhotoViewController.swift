...
class CameraPhotoViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        subscribe()
        photo_Image.image = img //отображение полученной ранее фотографии
...
//            обработка ошибок
            if let code = element.code, code < 200 || code >= 300, let msg = element.msg{
                if let ss = self{
                    ...
                    if let alert = Auxiliary_PoliDash.showMessage(vc: ss, msg: msg, tittle: "Ошибка", actionBtn: "ОК", callback: {
                        return
                    }){
