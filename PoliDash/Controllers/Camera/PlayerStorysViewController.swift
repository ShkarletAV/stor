...
//    обработчик нажатия на экран
    @objc func handleNavigation (sender: UITapGestureRecognizer) {
        let location = sender.location(in: self.overlayView)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if !self.isDoubleTap {
//                если было одно нажатие по правой стороне экрна
                print(self.overlayView.frame.size.width / 2)
                print(sender.location(in: self.overlayView).x)
...
//                    если фото то продолжаем воспроизведение и стираем окружность для поиска лайков
                    if let pContol = updateContolImage {
                        pContol(.play)
                        countHeart.text = "0"
                        if let count = sympathyData.count {
...
//        настрока сохранения контента в галерею
        ...
        self.addShadow(view: self.closeBtn)
        //self.addShadow(view: self.heartBtn)
...
//        настрока жестов
        ...
        tapRecognizer = UITapGestureRecognizer(
            target: self,
            action: #selector(handleNavigation(sender:))
        )
