...
class SimpleCollectionCell: UITableViewCell {
    ...
    var histories : [HistoryVideo]?{
        didSet {
            ...
            if let count = histories?.count, count > 0 {
...
extension SimpleCollectionCell : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        case .likes?:
            return self.likes?.count == 0 ? 2 : (self.likes?.count)! + 1
        case .followers?:
            return self.followers?.count == 0 ? 2 : (self.followers?.count)! + 1
        default:
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        switch self.delegate?.activityType {
        case .likes?:
            let cellID = indexPath.row == 0 ? "like_placeholder" : self.likes?.count == 0 ? "empty" : "cell"
            ...
            return cell
        case .followers?:
            let cellID = indexPath.row == 0 ? "followers_placeholder" : self.followers?.count == 0 ? "empty" : "cell"
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as! SCCell
            if cellID == "cell"{
                let follower = self.followers![indexPath.row-1] as! Owners_Model
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .followers?:
            ...
            self.delegate?.ownerUser_Action(self.followers![indexPath.row-1] as! Owners_Model)
        default:
            let history = histories![histories!.count - indexPath.row - 1]
            if history.preview == "placeholder" {
