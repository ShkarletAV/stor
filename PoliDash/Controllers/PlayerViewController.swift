...
class PlayerViewController: UIViewController {
    ...
    var swipeRecognizerRight  : UISwipeGestureRecognizer? = nil
    var swipeRecognizerDown  : UISwipeGestureRecognizer? = nil
    
    //MARK:- Request Download Video
    var msg_DownloadVideo = Variable<MessageVideoModel>(MessageVideoModel())
...
//    метод для сжатия видео по url
    private func compressVideo(inputURL: URL, quality: QualityAction, outputURL: URL) -> Void {
...
//        Наблюдатель результатов отправки видео
        msg_DownloadVideo.asObservable().skip(1).subscribe(onNext: { [weak self] (element) in
            if let code = element.code, code < 200 || code >= 300, let msg = element.msg{
                            self?.showAlertView(text: msg, callback: {})
                            }else{
                                if let code = element.code, code >= 200 && code < 300, let msg = element.msg{
                                    if let ss = self{
                                        if let alert = Auxiliary_PoliDash.showMessage(vc: ss, msg: msg, tittle: "", actionBtn: "ОК", callback: {
                                            if let sss = self{
