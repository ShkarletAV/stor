...
//            Запросить пользователя по нику
            ProfileAPI.requestSearchUsers(delegate: delegate, nickName: searchT, callback: {[weak self] (_, _, searchModel) in
...
//        Наблюдатель изменения модели пользователей
        searchUsers.asObservable().bind(to: tableViewUsers.rx.items) {
            ...
            cell.titleNameLabel.text = model.nickname
            cell.userButton.tag = row
            if let urlImg = model.picture {
                cell.userPhotoImage.sd_setImage(with: URL(string: "\(urlImg)"), placeholderImage: UIImage(named: "User_placeholder"), options: SDWebImageOptions(rawValue: 0), completed: nil)
            } else {
                cell.userPhotoImage.image = UIImage(named: "User_placeholder")
            }
...
//                Наблюдатель измения историй
                Observable.just(videos).asObservable().bind(to: cell.collectionViewStorys!.rx.items) {
                    ...
                    if let urlImg = data.preview {
                        cell2.imagePreviewImage.sd_setImage(with: URL(string: "\(urlImg)"), placeholderImage: UIImage(named: "Placeholder"), options: SDWebImageOptions(rawValue: 0), completed: nil)
                    } else {
                        cell2.imagePreviewImage.image = UIImage(named: "Placeholder")
                    }
                    return cell2
